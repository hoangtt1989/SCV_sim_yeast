...
if strcmp(type_ctrl.regType, 'pen') % regularization type: penalty or constraint
    ...
    Nu = 1e-4; %0; %1e-4; %0; %1e-4; %0; %1e-4; %1e-2; %   Nu: the ridge shrinkage parameter for the scaled model. That is, the
...
if strcmp(type_ctrl.probType, 'SEL') % problem type: SEL-RRR/PCA
    ...
    if strcmp(estCorrtWay, 'rrr')
        [Bs_corrt] = Func_EstsCorrt(Bs, X, Y, tuningparams, nzPatts, rankBs, numBs);
        Bs_orig = Bs;
        Bs = Bs_corrt;
    elseif strcmp(estCorrtWay, 'pca')
        [Bs_corrt] = Func_EstsCorrt_PCA(Bs, X, tuningparams, nzPatts, rankBs, numBs);
        Bs_orig = Bs;
        Bs = Bs_corrt;
    end
else
...
tmp_ranks = zeros(1, numBs);
for i = 1:numBs
    
    [~, singvals, ~] = svd(Bs(:, :, i), 'econ');
    singvals = diag(singvals);
    singvals(singvals<1e-4) = 0;
    inds = find(singvals > 0);
    
    tmp_ranks(i) = numel(inds);
