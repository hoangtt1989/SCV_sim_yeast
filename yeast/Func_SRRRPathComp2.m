...
if full2d == 1 % full 2D:
    ...
    for k = candranks
        for Lambda = k:nPoints %%%%%%%%this must be changed back if we want to do penalized type
...
%             Lambda = lambdas(lambdaInd);
            if rankGiven > 0 && rankGiven == min([size(X), size(Y)]) % just the group penalty; no rank constraint
                ...
                if exist('lambdasGiven', 'var') && ~isempty(lambdasGiven)
                    Lambda = lambdasGiven; %%%lambdasGiven must be a single value here
                end
            ...
            if exist('lambdasGiven', 'var') && ~isempty(lambdasGiven)
                break %%%lambdasGiven must be a single value here
            end
